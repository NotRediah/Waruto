...
exec_always swaybg -i ~/Pictures/forest.jpg
...
# Force the desktop ID to 'sway' so portals recognize it
...
client.focused          #b0b0b0 #222222 #ffffff #b0b0b0   #b0b0b0
client.focused_inactive #444444 #222222 #b0b0b0 #444444   #444444
client.unfocused        #222222 #111111 #888888 #222222   #222222 
...
#
    ...
    bindsym $mod+Alt+q kill unfocused
...
#
    ...
    bindsym --whole-window $mod+Shift+button4 focus left
    bindsym --whole-window $mod+Shift+button5 focus right
# for the mouse moving side to side you need to move mouse to the side you're gonna move to    
...
# Scaling
    # Workspace
    bindsym $mod+Alt+comma scale_workspace incr -0.05
    bindsym --whole-window $mod+Alt+button4 scale_workspace incr -0.05
    bindsym $mod+Shift+period scale_workspace incr 0.05
    bindsym --whole-window $mod+Alt+button5 scale_workspace incr 0.05
    bindsym $mod+Alt+Ctrl+period scale_workspace reset

    # Overview
    bindsym  $mod+o scale_workspace overview
    bindsym --whole-window button8 scale_workspace overview

    # Jump
      jump_labels_color #b0b0b0
    # jump_labels_background #15309eFF
    # jump_labels_scale 0.9
      jump_labels_keys asdf
    bindsym --no-repeat $mod+tab jump
    bindsym --no-repeat $mod+Shift+tab jump container
    bindsym --no-repeat $mod+Ctrl+tab jump workspaces
    bindsym --no-repeat $mod+Alt+tab jump floating

    # Content
    bindsym $mod+Alt+minus scale_content incr -0.05
    bindsym --whole-window $mod+Ctrl+button4 scale_content incr -0.05
    bindsym $mod+Alt+equal scale_content incr 0.05
    bindsym --whole-window $mod+Ctrl+button5 scale_content incr 0.05
    bindsym $mod+Ctrl+equal scale_content reset
...
bindsym $mod+p exec dms ipc call notepad toggle
bindsym $mod+x exec dms ipc call powermenu toggle
bindsym $mod+comma exec dms ipc call settings toggle
bindsym $mod+Alt+b exec dms ipc call bar toggle
bindsym $mod+Shift+s exec  dms screenshot -d ~/Pictures/Screenshots   
##specific app keybinds
bindsym $mod+Shift+V exec vesktop
bindsym $mod+Shift+O exec obsidian
bindsym $mod+SHift+f exec firefox

#WORKSPACE RULES (BWAHAHAHHA)
for_window [title="^Picture-in-Picture$"] floating enable, sticky enable, border none, resize set 400 225, move position 100 ppt 0, move left 410
## vesktop
assign [class="(?i)vesktop"] 2
assign [app_id="(?i)vesktop"] 2
## dms settings

for_window [app_id="org.quickshell" title="Settings"] floating enable, border none
